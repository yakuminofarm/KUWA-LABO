# くわらぼ

クワガタのブリード・飼育管理アプリ。成虫・幼虫・蛹の個体管理、ブリードラインの進行、
エサやりチェック、ビン交換記録、収支の集計をひとつにまとめている。

記録は端末のブラウザ (localStorage) にだけ保存される。飼育の記録がサーバーに
送られることはない。機種変更や端末をまたぎたいときは、アプリ内の
「データの持ち出し」から JSON で書き出して読みこむ。

例外として、ホーム画面のニュース欄 (下記) だけは外部と通信する。
固定の検索キーワードを送って見出しを取ってくるだけで、飼育記録などの
個人のデータは一切ふくまれない。

## 開発

```bash
npm install
npm run dev      # http://localhost:3000
npm run build    # 本番ビルド
npm run lint
npm test         # ロジックのテスト (Vitest)
```

## テスト

`src/**/*.test.ts(x)` に、計算まわりを中心にテストを置いている。
とくにお金がからむ箇所は、二重計上を防ぐガードそのものをテストしている。

- `src/lib/breeding.test.ts` — 収支の集計、頭数の計算、ひと月のめやす、
  単価の内訳など、純粋な計算関数
- `src/store/kuwagataStore.test.ts` — 幼虫→成虫の引き上げ・まとまりの
  切り出しで、費用を二重に写さないこと
- `src/components/BeetleFields.test.ts` — 個体の複製で入手金額を
  引き継がないこと、管理番号の自動採番

新しく計算やガードを足すときは、まずここに1本足す。UIの見た目は
毎回人が見て確かめてきたが、数字の整合はそれでは追いきれない。

## 金額の扱い

**記録する金額はすべて税込。** アプリ側で税を足したり引いたりはしない。
レシートの支払った額・受け取った額をそのまま持つのが収支を実際と
合わせるいちばん確実なやり方で、税率や内税/外税をアプリが推測すると
どこかで必ずずれるため。入力欄と金額の行にはその旨を出している
(「入手金額 (税込)」「単価 (税込)」など)。

金額に計算が入るのはペアの合計を2頭に割り振るときだけ
(`splitPairAmount`)。片方を切り上げ、もう片方は引き算で出すので、
足すと必ず元の金額に戻る。

## ホームのニュース欄

`src/app/api/news/route.ts` が、Google News の検索RSS (キー不要の公開
エンドポイント) からクワガタ関連の見出しをサーバー側で取ってくる。
取得結果は6時間ぶん使い回す (`fetch` の `next.revalidate`)。

取得に失敗しても (オフライン・取得先の不調など) `items: []` を返すだけで、
エラーにはしない。ホーム側 (`KuwagataHomeTab.tsx`) はニュースが無ければ
`src/lib/trivia.ts` の雑学に自動で切り替わるので、どちらの状態でも
ホーム画面の見た目は壊れない。

雑学のほうはすべてアプリ内に埋め込んだ静的な内容で、実在が確かめられた
記念日 (6/4 虫の日) と、出典つきで確認したクワガタの豆知識だけを載せている
(出典は `trivia.ts` 冒頭のコメントを参照)。ニュースと違って、こちらは
外部と通信しない。

## 配信先

| ブランチ | 用途 | 配信 |
| --- | --- | --- |
| `main` | 本番。実際の飼育記録を入れる場所 | kuwa-labo.vercel.app |
| `dev` | テスト。壊れても困らない場所 | Vercel の Preview |

`dev` で確かめてから `main` に取り込む。急ぎの修正だけ直接 `main` に入れる。

本番以外では、ヘッダーに「テスト」の印が出て、ホーム画面に置いたときの
名前も「くわらぼ試」になる。判定は `src/lib/env.ts` が Vercel のビルド時の
値を見ており、preview とローカルはどちらもテスト扱い。誤るなら本番を
テストと呼ぶ側に倒してある。逆だと、テストと気づかないまま本気の記録を
入れることになるため。

**記録は配信先ごとに完全に別**になる。localStorage はURLの単位で分かれる
ので、テストで入れたものは本番には出てこない。移すときはアプリ内の
「データの持ち出し」を使う。

なお Vercel は同じコミットを二度ビルドしない。`dev` が `main` と同じ
コミットを指している間は Preview が作られないので、確認したいときは
`dev` 側に差分が要る。

## 構成

| 場所 | 中身 |
| --- | --- |
| `src/app/` | ルート直下がアプリ本体。`icon.png` / `apple-icon.png` はホーム画面用 |
| `src/components/` | 画面パーツ。`tabs/` が下タブ6つ、`ui/` が汎用 |
| `src/lib/breeding.ts` | 蛹化・羽化・掘り出しの日数計算、ステージ定義、収支集計 |
| `src/lib/assets.ts` | イラストを data URI で抱えている (外部リクエストなしで表示するため) |
| `src/store/` | zustand + persist。localStorage への保存もここ |
| `src/types/` | 個体・ライン・幼虫などのドメイン型 |
| `public/sw.js` | オフライン用 Service Worker (本番ビルドでのみ登録) |

## オフライン対応

`public/sw.js` は画面本体を**ネット優先**で取りに行き、失敗したときだけキャッシュを返す。
`/_next/static/` 配下はファイル名にハッシュが入る前提で**キャッシュ優先**にしている。
更新を握りつぶさないための使い分けなので、この2つを入れ替えないこと。

## 共有用の1枚HTML

```bash
npm run artifact
```

`next.config.ts` に `output: "export"` を入れた状態でビルドし、CSS/JS を埋め込んだ
`kuwarabo.html` を書き出す。日本語フォントは約250サブセットで数MBになるため埋め込まず、
Google Fonts から読ませている。

## 出自

もとは [rises-fish](https://github.com/yakuminofarm/rises-fish) の `/kuwagata` に
めだか手帳と同居していたものを、独立したアプリとして切り出した。
